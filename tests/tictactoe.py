...
class TicTacToeState(GameState):
    ...
    def is_terminal(self):
        # Check for 3 in a row on rows and columns.
        for i in range(3):
            if abs(sum(self.board[i])) == 3 or abs(sum([row[i] for row in self.board])) == 3:
                return True
        # Check for 3 in a row on diagonals.
    ...
    def get_reward(self):
        win_sum = -self.player * 3 # "negative player" because we're finding the reward of the previous player (the one who made the terminal move)
        for i in range(3):
